...
// IRouter defines the interface for posting messages *to* a user-space
...
type IRouter interface {
	// PostMessage queues a message for the component to process in its
	// own event loop.
	PostMessage(msg IMessage)
}

// IKernelRequestRouter defines the interface for a user-space process to post
...
type IKernelRequestRouter interface {
	// PostKernelRequest queues a message for the kernel's central event loop
	// to process.
	PostKernelRequest(msg IMessage)
}

// IKernelResponseRouter defines the interface used by kernel-space servers (and the
// kernel itself) to post messages to a specific destination process via the kernel.
//
// Unlike IKernelRequestRouter, this interface requires a destination `pid` because
...
type IKernelResponseRouter interface {
	// PostKernelResponse queues a message for the kernel to deliver to the
	// process identified by `destination`.
	PostKernelResponse(destination int, msg IMessage)
}
