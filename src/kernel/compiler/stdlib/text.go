...
var _textModule = map[string]objects.IObject{
	...
	"compare":        objects.NewFunctionUser("compare", objects.FuncIssOi(strings.Compare)),             // compare(a, b) => int
	"contains":       objects.NewFunctionUser("contains", objects.FuncIssOb(strings.Contains)),           // contains(s, substr) => bool
	"contains_any":   objects.NewFunctionUser("contains_any", objects.FuncIssOb(strings.ContainsAny)),    // contains_any(s, chars) => bool
	"count":          objects.NewFunctionUser("count", objects.FuncIssOi(strings.Count)),                 // count(s, substr) => int
	"equal_fold":     objects.NewFunctionUser("equal_fold", objects.FuncIssOb(strings.EqualFold)),        // "equal_fold(s, t) => bool
	"fields":         objects.NewFunctionUser("fields", objects.FuncIsOsS(strings.Fields)),               // fields(s) => [string]
	"has_prefix":     objects.NewFunctionUser("has_prefix", objects.FuncIssOb(strings.HasPrefix)),        // has_prefix(s, prefix) => bool
	"has_suffix":     objects.NewFunctionUser("has_suffix", objects.FuncIssOb(strings.HasSuffix)),        // has_suffix(s, suffix) => bool
	"index":          objects.NewFunctionUser("index", objects.FuncIssOi(strings.Index)),                 // index(s, substr) => int
	"index_any":      objects.NewFunctionUser("index_any", objects.FuncIssOi(strings.IndexAny)),          // index_any(s, chars) => int
	"join":           objects.NewFunctionUser("join", textJoin),                                          // join(arr, sep) => string
	"last_index":     objects.NewFunctionUser("last_index", objects.FuncIssOi(strings.LastIndex)),        // last_index(s, substr) => int
	"last_index_any": objects.NewFunctionUser("last_index_any", objects.FuncIssOi(strings.LastIndexAny)), // last_index_any(s, chars) => int
	...
	"split":          objects.NewFunctionUser("split", objects.FuncIssOsS(strings.Split)),                // split(s, sep) => [string]
	"split_after":    objects.NewFunctionUser("split_after", objects.FuncIssOsS(strings.SplitAfter)),     // split_after(s, sep) => [string]
	"split_after_n":  objects.NewFunctionUser("split_after_n", objects.FuncIssiOsS(strings.SplitAfterN)), // split_after_n(s, sep, n) => [string]
	"split_n":        objects.NewFunctionUser("split_n", objects.FuncIssiOsS(strings.SplitN)),            // split_n(s, sep, n) => [string]
	"title":          objects.NewFunctionUser("title", objects.FuncIsOs(strings.Title)),                  // title(s) => string
	"to_lower":       objects.NewFunctionUser("to_lower", objects.FuncIsOs(strings.ToLower)),             // to_lower(s) => string
	"to_title":       objects.NewFunctionUser("to_title", objects.FuncIsOs(strings.ToTitle)),             // to_title(s) => string
	"to_upper":       objects.NewFunctionUser("to_upper", objects.FuncIsOs(strings.ToUpper)),             // to_upper(s) => string
	...
	"trim":           objects.NewFunctionUser("trim", objects.FuncIssOs(strings.Trim)),                   // trim(s, cutset) => string
	"trim_left":      objects.NewFunctionUser("trim_left", objects.FuncIssOs(strings.TrimLeft)),          // trim_left(s, cutset) => string
	"trim_prefix":    objects.NewFunctionUser("trim_prefix", objects.FuncIssOs(strings.TrimPrefix)),      // trim_prefix(s, prefix) => string
	"trim_right":     objects.NewFunctionUser("trim_right", objects.FuncIssOs(strings.TrimRight)),        // trim_right(s, cutset) => string
	"trim_space":     objects.NewFunctionUser("trim_space", objects.FuncIsOs(strings.TrimSpace)),         // trim_space(s) => string
	"trim_suffix":    objects.NewFunctionUser("trim_suffix", objects.FuncIssOs(strings.TrimSuffix)),      // trim_suffix(s, suffix) => string
	"atoi":           objects.NewFunctionUser("atoi", objects.FuncIsOie(strconv.Atoi)),                   // atoi(str) => int/error
	...
	"itoa":           objects.NewFunctionUser("itoa", objects.FuncIiOs(strconv.Itoa)),                    // itoa(i) => string
	...
	"quote":          objects.NewFunctionUser("quote", objects.FuncIsOs(strconv.Quote)),                  // quote(str) => string
	"unquote":        objects.NewFunctionUser("unquote", objects.FuncIsOse(strconv.Unquote)),             // unquote(str) => string/error
}
