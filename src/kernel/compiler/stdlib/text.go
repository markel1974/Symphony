...
var _textModule = map[string]objects.IObject{
	"re_match":       objects.NewFunctionModule(objects.FunctionModuleDef, "re_match", textREMatch),                                   // re_match(pattern, text) => bool/error
	"re_find":        objects.NewFunctionModule(objects.FunctionModuleDef, "re_find", textREFind),                                     // re_find(pattern, text, count) => [[{text:,begin:,end:}]]/undefined
	"re_replace":     objects.NewFunctionModule(objects.FunctionModuleDef, "re_replace", textREReplace),                               // re_replace(pattern, text, repl) => string/error
	"re_split":       objects.NewFunctionModule(objects.FunctionModuleDef, "re_split", textRESplit),                                   // re_split(pattern, text, count) => [string]/error
	"re_compile":     objects.NewFunctionModule(objects.FunctionModuleDef, "re_compile", textRECompile),                               // re_compile(pattern) => Regexp/error
	...
	"contains_any":   objects.NewFunctionModule(objects.FunctionModuleDef, "contains_any", objects.FuncIssOb(strings.ContainsAny)),    // contains_any(s, chars) => bool
	"count":          objects.NewFunctionModule(objects.FunctionModuleDef, "count", objects.FuncIssOi(strings.Count)),                 // count(s, substr) => int
	"equal_fold":     objects.NewFunctionModule(objects.FunctionModuleDef, "equal_fold", objects.FuncIssOb(strings.EqualFold)),        // "equal_fold(s, t) => bool
	"fields":         objects.NewFunctionModule(objects.FunctionModuleDef, "fields", objects.FuncIsOsS(strings.Fields)),               // fields(s) => [string]
	"has_prefix":     objects.NewFunctionModule(objects.FunctionModuleDef, "has_prefix", objects.FuncIssOb(strings.HasPrefix)),        // has_prefix(s, prefix) => bool
	"has_suffix":     objects.NewFunctionModule(objects.FunctionModuleDef, "has_suffix", objects.FuncIssOb(strings.HasSuffix)),        // has_suffix(s, suffix) => bool
	"index":          objects.NewFunctionModule(objects.FunctionModuleDef, "index", objects.FuncIssOi(strings.Index)),                 // index(s, substr) => int
	"index_any":      objects.NewFunctionModule(objects.FunctionModuleDef, "index_any", objects.FuncIssOi(strings.IndexAny)),          // index_any(s, chars) => int
	"join":           objects.NewFunctionModule(objects.FunctionModuleDef, "join", textJoin),                                          // join(arr, sep) => string
	"last_index":     objects.NewFunctionModule(objects.FunctionModuleDef, "last_index", objects.FuncIssOi(strings.LastIndex)),        // last_index(s, substr) => int
	"last_index_any": objects.NewFunctionModule(objects.FunctionModuleDef, "last_index_any", objects.FuncIssOi(strings.LastIndexAny)), // last_index_any(s, chars) => int
	"repeat":         objects.NewFunctionModule(objects.FunctionModuleDef, "repeat", textRepeat),                                      // repeat(s, count) => string
	"replace":        objects.NewFunctionModule(objects.FunctionModuleDef, "replace", textReplace),                                    // replace(s, old, new, n) => string
	"substr":         objects.NewFunctionModule(objects.FunctionModuleDef, "substr", textSubstring),                                   // substr(s, lower, upper) => string
	"split":          objects.NewFunctionModule(objects.FunctionModuleDef, "split", objects.FuncIssOsS(strings.Split)),                // split(s, sep) => [string]
	"split_after":    objects.NewFunctionModule(objects.FunctionModuleDef, "split_after", objects.FuncIssOsS(strings.SplitAfter)),     // split_after(s, sep) => [string]
	"split_after_n":  objects.NewFunctionModule(objects.FunctionModuleDef, "split_after_n", objects.FuncIssiOsS(strings.SplitAfterN)), // split_after_n(s, sep, n) => [string]
	"split_n":        objects.NewFunctionModule(objects.FunctionModuleDef, "split_n", objects.FuncIssiOsS(strings.SplitN)),            // split_n(s, sep, n) => [string]
	"title":          objects.NewFunctionModule(objects.FunctionModuleDef, "title", objects.FuncIsOs(strings.Title)),                  // title(s) => string
	"to_lower":       objects.NewFunctionModule(objects.FunctionModuleDef, "to_lower", objects.FuncIsOs(strings.ToLower)),             // to_lower(s) => string
	"to_title":       objects.NewFunctionModule(objects.FunctionModuleDef, "to_title", objects.FuncIsOs(strings.ToTitle)),             // to_title(s) => string
	"to_upper":       objects.NewFunctionModule(objects.FunctionModuleDef, "to_upper", objects.FuncIsOs(strings.ToUpper)),             // to_upper(s) => string
	"pad_left":       objects.NewFunctionModule(objects.FunctionModuleDef, "pad_left", textPadLeft),                                   // pad_left(s, pad_len, pad_with) => string
	"pad_right":      objects.NewFunctionModule(objects.FunctionModuleDef, "pad_right", textPadRight),                                 // pad_right(s, pad_len, pad_with) => string
	"trim":           objects.NewFunctionModule(objects.FunctionModuleDef, "trim", objects.FuncIssOs(strings.Trim)),                   // trim(s, cutset) => string
	"trim_left":      objects.NewFunctionModule(objects.FunctionModuleDef, "trim_left", objects.FuncIssOs(strings.TrimLeft)),          // trim_left(s, cutset) => string
	"trim_prefix":    objects.NewFunctionModule(objects.FunctionModuleDef, "trim_prefix", objects.FuncIssOs(strings.TrimPrefix)),      // trim_prefix(s, prefix) => string
	"trim_right":     objects.NewFunctionModule(objects.FunctionModuleDef, "trim_right", objects.FuncIssOs(strings.TrimRight)),        // trim_right(s, cutset) => string
	"trim_space":     objects.NewFunctionModule(objects.FunctionModuleDef, "trim_space", objects.FuncIsOs(strings.TrimSpace)),         // trim_space(s) => string
	"trim_suffix":    objects.NewFunctionModule(objects.FunctionModuleDef, "trim_suffix", objects.FuncIssOs(strings.TrimSuffix)),      // trim_suffix(s, suffix) => string
	"atoi":           objects.NewFunctionModule(objects.FunctionModuleDef, "atoi", objects.FuncIsOie(strconv.Atoi)),                   // atoi(str) => int/error
	"format_bool":    objects.NewFunctionModule(objects.FunctionModuleDef, "format_bool", textFormatBool),                             // format_bool(b) => string
	"format_float":   objects.NewFunctionModule(objects.FunctionModuleDef, "format_float", textFormatFloat),                           // format_float(f, fmt, prec, bits) => string
	"format_int":     objects.NewFunctionModule(objects.FunctionModuleDef, "format_int", textFormatInt),                               // format_int(i, base) => string
	"itoa":           objects.NewFunctionModule(objects.FunctionModuleDef, "itoa", objects.FuncIiOs(strconv.Itoa)),                    // itoa(i) => string
	"parse_bool":     objects.NewFunctionModule(objects.FunctionModuleDef, "parse_bool", textParseBool),                               // parse_bool(str) => bool/error
	"parse_float":    objects.NewFunctionModule(objects.FunctionModuleDef, "parse_float", textParseFloat),                             // parse_float(str, bits) => float/error
	"parse_number":   objects.NewFunctionModule(objects.FunctionModuleDef, "parse_number", textParseNumber),                           // parse_float(str, bits) => float/error
	"parse_int":      objects.NewFunctionModule(objects.FunctionModuleDef, "parse_int", textParseInt),                                 // parse_int(str, base, bits) => int/error
