...
var osModule = map[string]objects.IObject{
	...
	"chdir":               objects.NewFunctionUser("chdir", objects.FuncIsOe(os.Chdir)),                // chdir(dir string) => error
	"chmod":               osFuncASFmRE("chmod", os.Chmod),                                             // chmod(name string, mode int) => error
	"chown":               objects.NewFunctionUser("chown", objects.FuncIsiiOe(os.Chown)),              // chown(name string, uid int, gid int) => error
	"clearenv":            objects.NewFunctionUser("clearenv", objects.FuncInOn(os.Clearenv)),          // clearenv()
	"environ":             objects.NewFunctionUser("environ", objects.FuncInOsS(os.Environ)),           // environ() => array(string)
	"exit":                objects.NewFunctionUser("exit", objects.FuncIiOn(os.Exit)),                  // exit(code int)
	"expand_env":          objects.NewFunctionUser("expand_env", osExpandEnv),                          // expand_env(s string) => string
	"getegid":             objects.NewFunctionUser("getegid", objects.FuncInOi(os.Getegid)),            // getegid() => int
	"getenv":              objects.NewFunctionUser("getenv", objects.FuncIsOs(os.Getenv)),              // getenv(s string) => string
	"geteuid":             objects.NewFunctionUser("geteuid", objects.FuncInOi(os.Geteuid)),            // geteuid() => int
	"getgid":              objects.NewFunctionUser("getgid", objects.FuncInOi(os.Getgid)),              // getgid() => int
	"getgroups":           objects.NewFunctionUser("getgroups", objects.FuncInOiSe(os.Getgroups)),      // getgroups() => array(string)/error
	"getpagesize":         objects.NewFunctionUser("getpagesize", objects.FuncInOi(os.Getpagesize)),    // getpagesize() => int
	"getpid":              objects.NewFunctionUser("getpid", objects.FuncInOi(os.Getpid)),              // getpid() => int
	"getppid":             objects.NewFunctionUser("getppid", objects.FuncInOi(os.Getppid)),            // getppid() => int
	"getuid":              objects.NewFunctionUser("getuid", objects.FuncInOi(os.Getuid)),              // getuid() => int
	"getwd":               objects.NewFunctionUser("getwd", objects.FuncInOse(os.Getwd)),               // getwd() => string/error
	"hostname":            objects.NewFunctionUser("hostname", objects.FuncInOse(os.Hostname)),         // hostname() => string/error
	"lchown":              objects.NewFunctionUser("lchown", objects.FuncIsiiOe(os.Lchown)),            // lchown(name string, uid int, gid int) => error
	"link":                objects.NewFunctionUser("link", objects.FuncIssOe(os.Link)),                 // link(oldname string, newname string) => error
	...
	"readlink":            objects.NewFunctionUser("readlink", objects.FuncIsOse(os.Readlink)),         // readlink(name string) => string/error
	"remove":              objects.NewFunctionUser("remove", objects.FuncIsOe(os.Remove)),              // remove(name string) => error
	"remove_all":          objects.NewFunctionUser("remove_all", objects.FuncIsOe(os.RemoveAll)),       // remove_all(name string) => error
	"rename":              objects.NewFunctionUser("rename", objects.FuncIssOe(os.Rename)),             // rename(oldpath string, newpath string) => error
	"setenv":              objects.NewFunctionUser("setenv", objects.FuncIssOe(os.Setenv)),             // setenv(key string, value string) => error
	"symlink":             objects.NewFunctionUser("symlink", objects.FuncIssOe(os.Symlink)),           // symlink(oldname string newname string) => error
	"temp_dir":            objects.NewFunctionUser("temp_dir", objects.FuncInOs(os.TempDir)),           // temp_dir() => string
	"truncate":            objects.NewFunctionUser("truncate", objects.FuncIsi64Oe(os.Truncate)),       // truncate(name string, size int) => error
	"unsetenv":            objects.NewFunctionUser("unsetenv", objects.FuncIsOe(os.Unsetenv)),          // unsetenv(key string) => error
	...
	"exec_look_path":      objects.NewFunctionUser("exec_look_path", objects.FuncIsOse(exec.LookPath)), // exec_look_path(file) => string/error
