...
func (c *ControlFlow) TypeSwitchStmt(node *ast.TypeSwitchStmt) error {
	...
	for _, clauseStmt := range node.Body.List {
		...
		jumpToNextCasePos, err = c.scopes.Emit(bytecode.OpJumpFalsy, 9999)
		if err != nil {
			return err
		}
		...
		// ===== THE KEY FIX IS HERE (Thanks to your suggestion) =====
		...
		if lastInstructionInCase == nil || lastInstructionInCase.Opcode() != bytecode.OpReturn {
			jumpPos, err := c.scopes.Emit(bytecode.OpJump, 9999)
			if err != nil {
				return err
			}
	...
	if defaultClause := findDefault(node.Body); defaultClause != nil {
		for _, stmt := range defaultClause.Body {
			if err := c.compile(stmt); err != nil {
				return err
			}
...
func findDefault(list *ast.BlockStmt) *ast.CaseClause {
