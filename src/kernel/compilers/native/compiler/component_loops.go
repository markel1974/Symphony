...
func NewLoops(gk objects.IGateKeeper, scopes *tables2.Scopes, structTable *tables2.StructTable, functionTable *tables2.FunctionTable) *Loops {
	return &Loops{
		gk:            gk,
		scopes:        scopes,
...
func (c *Loops) RangeStmt(node *ast.RangeStmt) error {
	...
	default:
		return tables2.NewCompilerError(c.fileSet, node, "unsupported range expression: %T", node.X)
	}
