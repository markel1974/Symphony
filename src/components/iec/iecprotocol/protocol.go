...
func (v *Protocol) Emulate() {
	...
	if !conversion.Uint8ToBool(device.GetFlags()&P_ATN) && !conversion.Uint8ToBool(bus&IECBUS_DEVICE_READ_ATN) {
		//Falling flank on ATN (bus master addressing all devices) */
		device.SetStateMachine(P_PRE0)
		device.SetFlags(device.GetFlags() | P_ATN)
		device.SetPrimary(0)
	...
	} else if conversion.Uint8ToBool(device.GetFlags()&P_ATN) && conversion.Uint8ToBool(bus&IECBUS_DEVICE_READ_ATN) {
		...
		if (device.GetPrimary() == 0x20+deviceNumber) || (device.GetPrimary() == 0x40+deviceNumber) {
			if (device.GetSecondary() & 0xf0) == 0x60 {
				...
				v.gs.SetState(0)
				state := device.Close(device.GetSecondary())
				...
				v.gs.SetState(0)
				state := device.Open(device.GetSecondary())
				device.SetState(device.GetSecondary(), state)
				//device.SetState(device.GetSecondary(), v.gs.GetState())
			}
		...
		} else if (device.GetPrimary() == 0x5f) && conversion.Uint8ToBool(device.GetFlags()&P_TALKING) {
	...
	if conversion.Uint8ToBool(device.GetFlags() & (P_ATN | P_LISTENING)) {
		v.doListening(bus)
	} else if conversion.Uint8ToBool(device.GetFlags() & P_TALKING) {
		v.doTalking(bus)
	}
}
...
func (v *Protocol) doTalking(bus uint8) {
	...
	case P_READY:
		if conversion.Uint8ToBool(bus & IECBUS_DEVICE_READ_DATA) {
			...
			v.gs.SetState(device.GetState(device.GetSecondary()))
			b, state := device.Read(device.GetSecondary())
			device.SetByte(b)
			device.SetState(device.GetSecondary(), state)
