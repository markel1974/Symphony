...
func (f *Factory) Load(p string) (ICartridge, error) {
...
func (f *Factory) loadCrt(ldr *loader.CRTLoader) (ICartridge, error) {
	var crt ICartridge
	switch ldr.Kind {
	case loader.CARTRIDGE_OCEAN:
		crt = NewCartridgeOcean(uint8(ldr.Game), uint8(ldr.ExRom), ROM_LO, ROM_HI_1)
	case loader.CARTRIDGE_EASYFLASH:
		crt = NewCartridgeEasyFlash(uint8(ldr.Game), uint8(ldr.ExRom), ROM_LO, ROM_HI_1)
	}
	if crt == nil {
...
func (f *Factory) loadBin(ldr *loader.CRTLoader) (ICartridge, error) {
	var cart ICartridge = nil
	lCartridge := len(ldr.GetData())
	if lCartridge == 0x2000 {
		cart = NewCartridge8K(0, 1, ROM_LO)
	} else if lCartridge == 0x4000 {
		cart = NewCartridge16K(0, 0, ROM_LO, ROM_HI_1)
	} else if lCartridge > 0x4000 {
		//TODO VERIFICA
		//shadow of the beast funziona con ROM_LO, ROM_HI_1
		//robocop2 funziona con ROM_LO, ROM_HI_1
		cart = NewCartridgeOcean(0, 0, ROM_LO, ROM_HI_1)
	}
	if cart == nil {
