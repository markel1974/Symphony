...
func NewMOS6502(id string) *MOS6502 {
	cpu := &MOS6502{
		id:    id,
		prefs: nil,
		Core:  nil,
...
func (cpu *MOS6502) Setup(intr IPic, banks IBanks, prefs *config.Config) {
...
func (cpu *MOS6502) popFlags(data uint8) {
	cpu.nFlag = data
	cpu.vFlag = data & 0x40
	cpu.dFlag = data & 0x08
	cpu.iFlag = data & 0x04
...
func (cpu *MOS6502) buildFlags(bFlags bool) uint8 {
...
func (cpu *MOS6502) branch(data uint8) {
...
func (cpu *MOS6502) doADC(data uint8) {
	k := uint8(0)
	if cpu.cFlag != 0 {
		k = 1
	}
...
func (cpu *MOS6502) doSBC(data uint8) {
	k := uint8(0)
	if cpu.cFlag == 0 {
		k = 1
	}
...
func (cpu *MOS6502) illegalOp(illOp uint8, at uint16) {
...
func (cpu *MOS6502) GetState() uint8 {
	return cpu.state
}

func (cpu *MOS6502) SOBPin() {
	cpu.vFlag = 1
}

func (cpu *MOS6502) checkPic() {
	...
	if cpu.pic.HasNMI() {
		...
		if (cpu.pic.GetNMICycleDistance(0)) >= 2 {
	...
	if (cpu.pic.HasIRQ()) && (cpu.iFlag == 0) {
		// Taken branches to the same page delay the IRQ
		if (cpu.pic.GetIrqCycleDistance(0)) >= 2 {
			cpu.state = I_IRQ_8
		}
		return
	}
...
func (cpu *MOS6502) Emulate() {
	...
	case O_BVS:
		// TODO GCR byte ready flag PATCH
		tst := cpu.vFlag
		if pcr := cpu.banks.Read(0x1c0c) & 0x0e; pcr == 0x0e {
			tst = 1
		}
		data := cpu.banks.Read(cpu.pc)
		cpu.pc++
...
func (cpu *MOS6502) printRegisters(qCycle uint64, baLow bool) {
