...
// You can notice that there are no "drawing" methods on the internet.
// That's because all drawing happens indirectly through ITriangles and IPicture instances generated via
// MakeTriangles and MakePicture method.
type ITarget interface {
	...
	//
	// Note that not every ITarget has to recognize all possible types of ITriangles.
	// Some may only recognize ITrianglesPosition and ITrianglesColor and ignore all other properties (if present)
	// when making new ITargetTriangles.
	// This varies from ITarget to ITarget.
...
// weight of the IPicture. The Value of 0 means that IPicture should be completely ignored, 1 means that
// should be fully included and anything in-between-means anything in between.
...
type IPicture interface {
	...
	// Querying properties outside the rectangle should return the default value of that property.
	Bounds() Rect
}

// ITargetPicture is an IPicture generated by a ITarget using MakePicture method.
// This IPicture can be drawn onto
...
type ITargetPicture interface {
	...
	// this ITargetPicture) with this ITargetPicture. The ITargetTriangles should use the data
