...
const (
	ColorLevelNone = Level(iota)
	ColorLevelBasic
	ColorLevelHundreds
	ColorLevelMillions
)

var _hexTo256Table = map[string]uint8{
	"000000": 0,
	"800000": 1,
	"008000": 2,
	"808000": 3,
...
const _codeExpr = `\033\[[\d;?]+m`

var _enable = os.Getenv("NO_COLOR") == ""
var _codeRegex = regexp.MustCompile(_codeExpr)
var _colorLevel, _ = detectTermColorLevel()
...
func detectTermColorLevel() (level Level, needVTP bool) {
	...
	if termVal != "screen" {
		...
		if val == "JetBrains-JediTerm" {
			return ColorLevelMillions, isWin
		}
	}
	if level = detectColorLevelFromEnv(termVal, isWin); level == ColorLevelNone {
