...
func formatByteLength(format otoFormat) int {
	switch format {
...
func NewSineWave(freq float64, duration time.Duration, channelCount int, format otoFormat) *SineWave {
...
func play(ctx *oto.Context, freq float64, duration time.Duration, channelCount int, format otoFormat) *oto.Player {
	p := ctx.NewPlayer(NewSineWave(freq, duration, channelCount, format))
	p.Play()
	return &p
}
