...
func (s *CartridgeExternalCPU) Bind(board references.IExpansionC64, ldr references.ICartridgeLoaderC64) error {
	...
	cpu, err := s.GetFactory().Create(s, Identifier(), mos6510.Identifier(), instance)
	if err != nil {
		return err
	}
	...
	p, err := s.GetFactory().Create(s, Identifier(), pic_6510.Identifier(), instance)
	if err != nil {
		return err
	}
	...
	q, err := s.GetFactory().Create(s, Identifier(), quartz.Identifier(), instance)
	if err != nil {
		return err
	}
